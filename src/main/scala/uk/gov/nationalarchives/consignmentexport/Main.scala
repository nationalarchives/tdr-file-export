  ...
  override def main: Opts[IO[ExitCode]] =
     exportOps.map {
      case FileExport(consignmentId) => for {
        ...
        _ <- bashCommands.runCommand(s"tar --sort=name --owner=root:0 --group=root:0 --mtime ${java.time.LocalDate.now.toString} -c ${config.efs.rootLocation}/$consignmentId | gzip -n > $tarPath")
